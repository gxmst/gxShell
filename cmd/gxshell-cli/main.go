...
func listServers() {
	...
	if !ok || len(servers) == 0 {
		fmt.Println("No AI/CLI-enabled servers configured.")
		fmt.Println("Enable AI/CLI access on a server profile in gxShell first.")
		return
	}
...
func showStatus() {
	...
	if !ok || len(active) == 0 {
		fmt.Println("No active AI/CLI SSH connections.")
		return
	}

	fmt.Printf("Active AI/CLI SSH connections (%d):\n\n", len(active))
...
COMMANDS:
  exec <server> "<command>"    Execute a command on an AI/CLI-enabled server
  list                         List AI/CLI-enabled server aliases
  status                       Show active AI/CLI SSH connections
...
SECURITY:
  ...
  - Server profiles are hidden unless AI/CLI access is enabled.
