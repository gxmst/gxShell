...
- Local document grants capture the directory identity for the session. Reads, PDF streams and atomic text replacement use Go directory-root operations, reject links/special files, and cannot follow a later symlink/junction outside that root. Replacing an authorized directory requires explicitly reopening the document. Markdown relative resources use the same directory boundary.
- Text reads reject invalid UTF-8 and binary control bytes before crossing the renderer boundary. Local and remote text saves recheck the existing file as well as the new text, so a file replaced by a binary after preview is not silently overwritten. Filename variants such as `.env.*` do not bypass content validation. Remote rereads add network IO and do not provide an atomic snapshot against another remote writer.
