...
- “限时自动化信任”持续 1、4、8 或 24 小时，只自动放行 T1。T2 始终需要点击原生框；
  T3 不进入批量“全部允许”，无论是否信任，每条命令都必须单独点击确认。
...
文件传输的确认只覆盖“移动文件”，不覆盖“执行文件”。限时自动化信任只自动放行 T1；
解释器、上传脚本和其他不透明执行至少是 T2，仍会弹框。这个机制约束的是守规矩的
