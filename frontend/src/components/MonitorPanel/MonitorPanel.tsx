...
export function MonitorPanel({ active, locale, onStart, onCpuClick, onPingClick, onDiskClick, onMemClick, onNetworkClick }: { active?: Tab; locale?: string; onStart: () => void; onCpuClick?: () => void; onPingClick?: () => void; onDiskClick?: () => void; onMemClick?: () => void; onNetworkClick?: () => void }) {
  ...
  // Rolling history for the sparklines. Same event, separate hook, so the panel
  // keeps its shape when a session has no history yet.
  const history = useMetricsHistory(active?.id);
  ...
  return (
    <div className="monitor-panel">
      ...
      {metrics && (
        <>
          <MetricRow icon={<Gauge size={12} />} label={t(lang, "cpu")} value={metrics.cpuPercent} series={history.map((s) => s.cpuPercent)} seriesLabel={t(lang, "historyLabel", { metric: t(lang, "cpu") })} clickable onClick={onCpuClick} />
          <MetricRow icon={<MemoryStick size={12} />} label={t(lang, "mem")} value={metrics.memoryPercent} detail={`${metrics.memoryUsedMb || 0}/${metrics.memoryTotalMb || 0} MB`} series={history.map((s) => s.memoryPercent)} seriesLabel={t(lang, "historyLabel", { metric: t(lang, "mem") })} clickable onClick={onMemClick} />
...
function MetricRow({ icon, label, value, detail, series, seriesLabel, clickable, onClick }: { icon: JSX.Element; label: string; value: number; detail?: string; series?: number[]; seriesLabel?: string; clickable?: boolean; onClick?: () => void }) {
  ...
  return (
    <div className={clsx("metric-row", clickable && "metric-row-clickable")} onClick={clickable ? onClick : undefined}>
      ...
      <div className="metric-row-content">
        ...
        {/* The meter shows the current value, the sparkline the recent shape.
            Only render the curve once there is history worth a line, so the row
            keeps its existing height on a freshly connected session. */}
        <div className="meter w-full"><div className={clsx("meter-fill", `meter-${tone}`)} style={{ width: `${safe}%` }} /></div>
        {series && series.length > 1 && (
          <Sparkline className={clsx("metric-spark", `metric-spark-${tone}`)} values={series} max={100} height={16} label={seriesLabel} />
        )}
      </div>
    </div>
  );
