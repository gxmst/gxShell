...
import { ChevronDown, Columns2, PanelLeft, Plus, RefreshCw, Rows2, Terminal, X } from "lucide-react";
import type { SplitDirection, Tab } from "../../types";
import { stateClass } from "../../utils/format";
import { types } from "../../../wailsjs/go/models";
import { t } from "../../i18n";

export function TabBar({ tabs, activeTab, profiles, sidebarCollapsed, onToggleSidebar, onActive, onClose, onReconnect, onTearOff, onSplitToggle, onNewConnection, onNewLocal, language }: { tabs: Tab[]; activeTab: string; profiles: types.Profile[]; sidebarCollapsed: boolean; onToggleSidebar: () => void; onActive: (id: string) => void; onClose: (id: string) => void; onReconnect: (tab: Tab) => void; onTearOff?: (tab: Tab) => void; onSplitToggle?: (tabId: string, direction: SplitDirection) => void; onNewConnection?: () => void; onNewLocal?: () => void; language?: string }) {
  ...
  return (
    <div className="tabbar">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button key={tab.id} title={full} className={clsx("tab", activeTab === tab.id && "tab-active")} onClick={() => onActive(tab.id)} onMouseDown={(e) => onTabMouseDown(e, tab)}>
              {tab.local ? <Terminal size={12} className="text-accent opacity-70 shrink-0" /> : <span className={clsx("status-dot", stateClass(tab.state))} />}
      ...
      <div className="tab-actions">
        ...
        <button className="tab-action" disabled={!active || active.local} onClick={() => active && onReconnect(active)}><RefreshCw size={14} /></button>
      </div>
    </div>
  );
}
